...
### 1.1 2026-08-03 实施顺序调整

用户明确要求先跳过“真实飞书测试消息”和“官方长连接接收”两项。本阶段保留现有官方长连接实现和接口兼容性，但不配置凭证、不启动连接器、不进行真实租户验收，也不以长连接成功作为下游开发前置条件。

本阶段开发入口改为已经落入 PostgreSQL 的授权消息，以及通过现有内部接入契约写入的非敏感测试事件。优先完成：

```text
消息与附件落库
→ 附件正文提取
→ Codex 版本/认证状态与 Attempt fencing
→ MessageCandidate
→ MatterUpdateProposal / LegalMatter
→ WorkItem 生命周期
→ 今日工作台
→ PostgreSQL 恢复和 Mac 可靠性
```

真实飞书消息和官方长连接现场验收作为后置阶段单独恢复；在执行前始终显示 `not_executed`，不得声称通过。
...
1. Codex 版本、认证状态与 Attempt fencing；
2. 附件落库、解析、片段和引用；
3. Candidate、MatterUpdateProposal 和 WorkItem 生命周期；
4. 今日工作台、评测和 Mac 常驻可靠性；
5. 本地配置、授权范围和集成向导；
6. 后置恢复真实飞书消息和官方长连接验收。
...
本节设计继续作为最终目标保留，但不属于当前第一实施波次。当前开发只修复下游能力所必需的数据兼容或测试注入边界，不启动真实连接器。
...
真实联调脚本必须显式启用，并输出结构化、脱敏报告。当前阶段无论凭证状态如何，真实飞书测试群和官方长连接验收均按用户要求报告 `not_executed`。下游链路使用非敏感测试事件、Fake 适配器和真实 PostgreSQL 验证，不把这些结果写成真实飞书通过。用户恢复该阶段后，再执行测试群到 WorkItem 的完整链路。
...
当前阶段可在真实飞书和官方长连接明确标记 `not_executed` 的前提下完成下游开发，但不得宣称整条真实飞书闭环完成。附件解析、Codex 状态或真实冒烟结果、Candidate 人工闭环、Matter 提案审批、WorkItem 生命周期、真实数据首页、Redis/租约恢复、Mac 自检、迁移往返、完整 CI、推送分支和 Draft PR 均需对应证据。任何未执行的真实飞书、真实 Codex 或人工未读验收必须单独列为未完成内容。
