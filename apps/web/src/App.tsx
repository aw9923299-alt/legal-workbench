...
const navItems = [
  ...
  { key: 'inbox', icon: <InboxOutlined />, label: <span>AI 收件箱 <Badge count={6} size="small" /></span> },
  { key: 'my-tasks', icon: <AuditOutlined />, label: '我的任务' },
  { key: 'all-tasks', icon: <AppstoreOutlined />, label: '全部任务' },
...
export default function RootApp() {
  ...
  const renderPage = () => {
    if (selectedTask) return <TaskDetailPage task={selectedTask} onBack={() => setSelectedTask(null)} />;
    if (page === 'dashboard') return <DashboardPage onOpenTask={openTask} />;
    if (page === 'inbox') return <InboxPage />;
    if (page === 'my-tasks' || page === 'all-tasks') return <TaskCenterPage onOpenTask={openTask} />;
    if (page === 'library') return <LibraryPage />;
    if (page === 'agents') return <AgentCenterPage />;
    if (page === 'security') return <SecurityPage />;
    return <div className="page placeholder-page"><h2>模块已预留</h2><p>该模块将在正式接入合同文件、系统设置或专项业务能力后启用。</p></div>;
  };
  ...
  return (
    <Layout className="app-shell">
      <Sider width={228} collapsedWidth={72} collapsed={collapsed} className="app-sider" trigger={null}>
        ...
        <Menu mode="inline" selectedKeys={[page]} items={navItems} onClick={({ key }) => { setPage(key); setSelectedTask(null); }} />
        <div className="sider-footer"><div className="sync-dot" />{!collapsed && <span>飞书同步正常</span>}</div>
      </Sider>
      <Layout>
        <Header className="app-header">
          <Button type="text" icon={collapsed ? <MenuUnfoldOutlined /> : <MenuFoldOutlined />} onClick={() => setCollapsed((value) => !value)} />
          <Input className="global-search" prefix={<SearchOutlined />} placeholder="搜索任务、消息、合同、主播或输入 / 打开命令" />
          <div className="header-actions">
            <Button icon={<PlusOutlined />}>快速创建</Button>
            <Tooltip title="飞书同步状态"><Button type="text" icon={<SafetyCertificateOutlined />}><span className="header-status">已同步</span></Button></Tooltip>
